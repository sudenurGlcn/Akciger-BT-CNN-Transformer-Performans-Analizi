...
SAVE_BASE_DIR = r'D:\Grup-1-LungClassification\Models\CoatNet-V1-IQ-QTHNCCD-Dataset'
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Epoch {epoch + 1} duration: {duration:.2f} seconds')
...
for fold in FOLDS:
    ...
    model = create_model('coatnet_1_224', pretrained=False, num_classes=len(class_names))
...
with open(os.path.join(RESULTS_DIR, 'classification_report_avg.txt'), 'w') as f:
    f.write(avg_report)
    f.write(f"\nAverage Accuracy: {avg_accuracy:.2f}%\n")
    f.write(f"Total training time across all folds: {total_all_training_time:.2f} seconds\n")